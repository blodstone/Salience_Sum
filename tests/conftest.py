...
from typing import Set, Dict, Iterable, Union, Any, Callable
...
class ModelTestCase:
    ...
    def __init__(self, param_file, dataset_file, save_path):
        # pylint: disable=attribute-defined-outside-init
        self.param_file = param_file
        self.params = Params.from_file(param_file)
        self.params['train_data_path'] = str(dataset_file['train'])
        self.params['validation_data_path'] = str(dataset_file['validation'])
    ...
    def ensure_model_can_train_save_and_load(self,
        ...
        archive_file = save_path / "model.tar.gz"
        copy_params = Params(copy.deepcopy(self.params.as_dict()))
        model = train_model(self.params, save_path)
        # model = train_model_from_file(param_file, save_path, overrides=overrides)
        ...
        for key in state_keys:
            # noinspection PyUnresolvedReferences
            assert_allclose(model.state_dict()[key],
                            loaded_model.state_dict()[key])
        reader = DatasetReader.from_params(copy_params['dataset_reader'])

        # Need to duplicate params because Iterator.from_params will consume.
        iterator_params = copy_params['iterator']
        ...
        # the same result out.
        model_dataset = reader.read(copy_params['validation_data_path'])
        # noinspection PyUnresolvedReferences
        iterator.index_with(model.vocab)
        model_batch = next(iterator(model_dataset, shuffle=False))

        loaded_dataset = reader.read(copy_params['validation_data_path'])
        ...
        # Both outputs should have the same states and the values for these states should be close.
        ...
        for i, instance_predictions in enumerate(single_predictions):
            for key, single_predicted in instance_predictions.items():
                ...
                if isinstance(single_predicted, torch.Tensor):
                    ...
                    assert_allclose(single_predicted.data,
                                    batch_predicted.data,
                                    atol=tolerance)
                else:
    ...
    def assert_fields_equal(self, field1, field2, name: str, tolerance: float = 1e-6) -> None:
        if isinstance(field1, torch.Tensor):
            ...
            assert_allclose([field1], [field2], rtol=tolerance, atol=tolerance)
        else:
            if field1 != field2:
...
@pytest.fixture
def model() -> Callable[[Path, Dict, Path], ModelTestCase]:
    def _model_factory(param_file, dataset_file, save_path):
        return ModelTestCase(param_file, dataset_file, save_path)
