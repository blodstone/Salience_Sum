...
python $HOME/summarize.py -input $DATA/ready/test.salience.tsv -vocab_path $MODEL/vocabulary -model $MODEL/pick.th -model_config $CONFIG/config.json -output_path $DATA/result_4/std_pg_clean_200.out -batch_size 24 --cuda --use_salience --use_constraint

MODEL=$HOME/model/bbc/pg_clean_16/300
CONFIG=$HOME/model_config/pg_clean_16/std_beam/300
python $HOME/summarize.py -input $DATA/ready/test.salience.tsv -vocab_path $MODEL/vocabulary -model $MODEL/pick.th -model_config $CONFIG/config.json -output_path $DATA/result_4/std_pg_clean_300.out -batch_size 24 --cuda --use_salience --use_constraint
